...
//
	public boolean addEmployee(Employee employee) {
		...
		while (orgChart.contains(employee) == false) {
			if (employee.hasManager() == true && !orgChart.contains(employee.getManager())) {
				orgChart.add(employee.getManager());
				orgChart.add(employee);
				return true;
			} else if (employee.hasManager() == false && employee instanceof Manager) {
				orgChart.add(employee);
				return true;
			}
		}
...
//    	get every one in the orgchart
//    	check which one is a manager and add to a new manager list
...
//get all employees, find empoyees with same manager and add to new hashset
		for (Employee e : orgChart) {
...
//	get  all employees in the orgchat
//	Set managers as keys and employees as values
	...
	public Map<Manager, Set<Employee>> getFullHierarchy() {
		...
		for (Manager m : getAllManagers()) {
			fullHierarchy.put(m, getDirectSubordinates(m));
